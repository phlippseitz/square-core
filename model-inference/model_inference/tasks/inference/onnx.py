...
class Onnx(Transformer):
    ...
    def _load_model(self, model_name, load_quantized=False, decoder_name=None):
        ...
        try:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained("UKP-SQuARE/"+model_name)    
        except EnvironmentError:
            # if tokenizer is not available in the repository, use the base model's tokenizer
            config_path = download_model(repo_id="UKP-SQuARE/"+model_name, filename="config.json")
        ...
        if self.is_encoder_decoder:
            ...
            decoder_path = download_model(repo_id="UKP-SQuARE/"+decoder_name, filename=filename)
